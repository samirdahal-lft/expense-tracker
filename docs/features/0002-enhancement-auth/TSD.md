...
| Data / State | Reads the user record (email → account, verifier check); no new persistent schema beyond session state associating a session with one `user_id`. Logout terminates that association. |
