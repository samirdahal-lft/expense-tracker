## Task T-golden-button — The Export CSV button renders as a filled golden control
...
- [ ] AC-1 [behavior]: The export control renders as a **filled golden** button — a light golden
      background with a **near-black** label on it — instead of today's outline treatment (a border
      over a transparent background). Dark-on-light is the rule, not a variant: white on a golden
      fill fails WCAG AA at this text size (TSD Grounding).
- [ ] AC-2 [behavior]: The control declares a **deeper golden** hover fill, so hover feedback
      survives the loss of the outline button's `hover:bg-secondary`.
...
- [ ] AC-6 [e2e]: In the running app (`docker compose up`, `http://localhost:8090`) the owner sees a
      golden Export CSV button, hover deepens it, tabbing to it shows a focus indicator, and
      clicking it still downloads the CSV — checked in both light and dark theme.

**End-to-end AC:** AC-6 [e2e] — reachable through the running app (required: green component/unit ≠ reachable)

> **AC-6 is human-only and cannot be automated here.** Whether a fill *reads as gold* is only
> observable to an eye on a real screen; jsdom has no layout or paint. AC-1..AC-4 are provable at the
> unit level only as "which fill/foreground/hover/focus classes were declared" — the contrast ratios
> themselves are fixed in the TSD by measurement, not asserted at runtime. Do not expect the ledger
> to prove the colour looks right.
